...
def get_args_parser():
    ...
    parser.add_argument(
        "--data_path",
        default="/scratch/shashank/dataset/WT_videos/",
        type=str,
        help="Please specify path to the ImageNet training data.",
    )
    parser.add_argument(
        "--output_dir",
        default="/scratch/shashank/checkpoint/dino_WT/vanilla/",
        type=str,
...
def train_one_epoch(
    ...
    for it, (images) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            teacher_global, teacher_patches, attn, query, key = teacher(
                images[:1], return_track=True
            )  # only the 2 global views pass through the teacher
            # masked_img = utils.attention_tracking(attn, query, key, args.patch_size, images[:2])
            masked_img = utils.MOT(
                attn, query, key, teacher_patches, args.patch_size, images[:1]
            )
            # masked_img = utils.attention_masking(attn, args.patch_size images[:2])
            for msk in range(masked_img.shape[0]):
                student_masked_global.append(
                    student(masked_img[msk, :], return_track=False)
                )
            student_local = student(images[1:], return_track=False)

            loss = dino_loss(
                student_local, student_masked_global, teacher_global, epoch
            )
            ...
            for cnt in range(num_frame):
                ...
                for iq, q in enumerate(teacher_out):
                    for v in range(len(student_masked)):
                        loss = torch.sum(
                            -q * F.log_softmax(student_masked[v], dim=-1), dim=-1
                        )
        ...
        self.local_transfo = transforms.Compose(
            [
                ...
                transforms.RandomResizedCrop(
                    112, scale=local_crops_scale, interpolation=Image.BICUBIC
                ),
                flip_and_color_jitter,
